...
! use Constants
 ...
 subroutine unfoldings_of_tensor5
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     ...
     .le. &
     (unfoldedTensor.absdiff.correctTensor) )) then
      print *, " *Assert_Equal_Within failed* in test unfoldings_of_tensor5 &
              &[Tensor_Class.fun:273]"
      print *, "  ", "unfoldedTensor.absdiff.correctTensor (",unfoldedTensor.absdiff.correctTensor,") is not", &
 0.0d0,"within",1.0e-8
      print *, ""
      noAssertFailed = .false.
 ...
 end subroutine unfoldings_of_tensor5
 ...
 subroutine Singular_Value_Decomposition_T3
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-3) &
     .ge. &
     (correctUs(1).absdiff.(theUMatrices(1))) &
             .and. &
     (0.0d0 &
     -1.0e-3) &
     .le. &
     (correctUs(1).absdiff.(theUMatrices(1))) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T3 &
              &[Tensor_Class.fun:390]"
      print *, "  ", "correctUs(1).absdiff.(theUMatrices(1)) (",correctUs(1).absdiff.(theUMatrices(1)),") is not", &
 0.0d0,"within",1.0e-3
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-3) &
     .ge. &
     (correctUs(2).absdiff.theUMatrices(2)) &
             .and. &
     (0.0d0 &
     -1.0e-3) &
     .le. &
     (correctUs(2).absdiff.theUMatrices(2)) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T3 &
              &[Tensor_Class.fun:391]"
      print *, "  ", "correctUs(2).absdiff.theUMatrices(2) (",correctUs(2).absdiff.theUMatrices(2),") is not", &
 0.0d0,"within",1.0e-3
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-3) &
     .ge. &
     (correctUs(3).absdiff.theUMatrices(3)) &
             .and. &
     (0.0d0 &
     -1.0e-3) &
     .le. &
     (correctUs(3).absdiff.theUMatrices(3)) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T3 &
              &[Tensor_Class.fun:392]"
      print *, "  ", "correctUs(3).absdiff.theUMatrices(3) (",correctUs(3).absdiff.theUMatrices(3),") is not", &
 0.0d0,"within",1.0e-3
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
...
!   assert_equal_within(correctCore.absdiff.theCore, 0.0d0, 1.0e-10)
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-10) &
     .ge. &
     (theTensor.absdiff. reconstructed) &
             .and. &
     (0.0d0 &
     -1.0e-10) &
     .le. &
     (theTensor.absdiff. reconstructed) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T3 &
              &[Tensor_Class.fun:398]"
      print *, "  ", "theTensor.absdiff. reconstructed (",theTensor.absdiff. reconstructed,") is not", &
 0.0d0,"within",1.0e-10
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
  ...
  if (noAssertFailed) then
    if (WasThereError()) then
      print *, " *Assert_False failed* in test Singular_Value_Decomposition_T3 &
              &[Tensor_Class.fun:400]"
      print *, "  ", "WasThereError() is not false"
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine Singular_Value_Decomposition_T3



 subroutine Singular_Value_Decomposition_T4
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     ...
     .le. &
     (theTensor.absdiff. reconstructed) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T4 &
              &[Tensor_Class.fun:417]"
      print *, "  ", "theTensor.absdiff. reconstructed (",theTensor.absdiff. reconstructed,") is not", &
 0.0d0,"within",1.0e-4
      print *, ""
      noAssertFailed = .false.
  ...
  if (noAssertFailed) then
    if (WasThereError()) then
      print *, " *Assert_False failed* in test Singular_Value_Decomposition_T4 &
              &[Tensor_Class.fun:419]"
      print *, "  ", "WasThereError() is not false"
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine Singular_Value_Decomposition_T4


 subroutine Singular_Value_Decomposition_T5
...
!    theTensor=new_Tensor(dim1,dim2,dim3,dim4,dim5)
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     ...
     .le. &
     (theTensor.absdiff. reconstructed) )) then
      print *, " *Assert_Equal_Within failed* in test Singular_Value_Decomposition_T5 &
              &[Tensor_Class.fun:447]"
      print *, "  ", "theTensor.absdiff. reconstructed (",theTensor.absdiff. reconstructed,") is not", &
 0.0d0,"within",1.0e-4
      print *, ""
      noAssertFailed = .false.
  ...
  if (noAssertFailed) then
    if (WasThereError()) then
      print *, " *Assert_False failed* in test Singular_Value_Decomposition_T5 &
              &[Tensor_Class.fun:449]"
      print *, "  ", "WasThereError() is not false"
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine Singular_Value_Decomposition_T5
...
!Flatten[Transpose[math, {3, 2, 1}]]
 ...
 subroutine nModeProducts_3
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-8) &
     .ge. &
     (T3_result.absdiff.T3_correct) &
             .and. &
     (0.0d0 &
     -1.0e-8) &
     .le. &
     (T3_result.absdiff.T3_correct) )) then
      print *, " *Assert_Equal_Within failed* in test nModeProducts_3 &
              &[Tensor_Class.fun:602]"
      print *, "  ", "T3_result.absdiff.T3_correct (",T3_result.absdiff.T3_correct,") is not", &
 0.0d0,"within",1.0e-8
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine nModeProducts_3
...
!end test
 ...
 subroutine nModeProducts_5
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     +1.0e-8) &
     .ge. &
     (T5_result.absdiff.T5_correct) &
             .and. &
     (0.0d0 &
     -1.0e-8) &
     .le. &
     (T5_result.absdiff.T5_correct) )) then
      print *, " *Assert_Equal_Within failed* in test nModeProducts_5 &
              &[Tensor_Class.fun:645]"
      print *, "  ", "T5_result.absdiff.T5_correct (",T5_result.absdiff.T5_correct,") is not", &
 0.0d0,"within",1.0e-8
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine nModeProducts_5
 ...
 subroutine tensor5JoinandSplit
  ...
  if (noAssertFailed) then
    if (.not.((0.0d0 &
     ...
     -1.0e-10) &
     .le. &
     (t5.absdiff.newT5) )) then
      print *, " *Assert_Equal_Within failed* in test tensor5JoinandSplit &
              &[Tensor_Class.fun:718]"
      print *, "  ", "t5.absdiff.newT5 (",t5.absdiff.newT5,") is not", &
 0.0d0,"within",1.0e-10
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
  ...
  if (noAssertFailed) then
    if (WasThereError()) then
      print *, " *Assert_False failed* in test tensor5JoinandSplit &
              &[Tensor_Class.fun:719]"
      print *, "  ", "WasThereError() is not false"
      print *, ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine tensor5JoinandSplit
 ...
 subroutine test_Tensor_Class( nTests, nAsserts, nAssertsTested, nFailures )
  ...
  call funit_setup
  call Singular_Value_Decomposition_T3
  call funit_teardown

  call funit_setup
  call Singular_Value_Decomposition_T4
  call funit_teardown

  call funit_setup
  call Singular_Value_Decomposition_T5
  call funit_teardown
  ...
  call funit_setup
  call nModeProducts_3
  call funit_teardown

  call funit_setup
  call nModeProducts_5
  call funit_teardown
